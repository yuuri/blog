...
```shell
iptables -t nat -A PREROUTING -d 192.168.122.3.70 -p tcp --dport 9200  -j DNAT --to-dest 192.168.122.100：9200
```
...
[1].https://wangchujiang.com/linux-command/c/iptables.html  (**iptables命令** 是Linux上常用的防火墙软件，是netfilter项目的一部分。可以直接配置，也可以通过许多前端和图形界面配置)

[2].https://linux.die.net/man/8/iptables (iptables man page)